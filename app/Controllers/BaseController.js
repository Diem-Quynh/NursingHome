...
class BaseController {
  ...
  async update({ allowFields = {} } = {}) {
    ...
    return await this.Model.findById(id).patch(data)
  }
  ...
  async destroy() {
    ...
    let [err, rs] = await to(this.Model.deleteById(id));
  ...
  async delete({ request, response }) {
    ...
    console.log(ids)
    if (_.isEmpty(ids) || !Array.isArray(ids)) throw new ApiException(9996, "ID is required. Expected: Array!");
    await this.Model.deleteByIds(ids);
    return true
  }
      ...
      if (isRequired && !isExists) { //nếu field là bắt buộc như lại không tồn tại trong data.
        ...
        if (!typeAllowed) {
          if (typeOfField == "any") {
            ...
            if (typeAllowed) _.set(newData, path, new Date(moment(data).add(7,'h')))
          }
