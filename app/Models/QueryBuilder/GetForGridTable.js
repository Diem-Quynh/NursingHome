...
class GetForGridTable {
  ...
  buildFilters() {
    for (let filter of this.filters) {
      ...
      switch (operator) {
        case 'contains':
          this.builder.where(columnName, 'LIKE', `%${value.replace(/%/, '\%')}%`)
          debug(`add filter contains: ${columnName} LIKE %${value}%`)
          break;
        case 'startWiths':
          this.builder.where(columnName, 'LIKE', `${value.replace(/%/, '\%')}%`)
          debug(`add filter startWiths: ${columnName} LIKE ${value}%`)
          break;
        case 'endWiths':
          this.builder.where(columnName, 'LIKE', `%${value.replace(/%/, '\%')}`)
          debug(`add filter endWiths: ${columnName} LIKE %${value}`)
          break;
        case 'between':
